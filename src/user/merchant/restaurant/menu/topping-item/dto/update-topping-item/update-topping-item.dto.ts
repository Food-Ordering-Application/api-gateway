...
export class UpdateToppingItemFullDto {
  @ApiProperty({
    ...
    required: true,
    nullable: false,
    description: 'Mã nhóm topping',
  })
  @IsUUID()
  toppingGroupId: string;

  @ApiProperty({
    example: 'Nước sốt Hàn Quốc',
    required: true,
  ...
  @ApiProperty({
    example: 'Nước sốt Hàn Quốc vị cay',
    required: true,
    description: 'Mô tả topping',
  })
  @IsString()
  description: string;
  ...
  @ApiProperty({
    example: 1,
    required: true,
    description: 'Số lượng topping tối đa cho một món',
  })
  @IsNumber()
  maxQuantity: number;
